...
export function mapForecast(tempUnit, forecast){
  ...
  const forecastList = forecast.map(item => {
    ...
    if (Number(forecastDay) > currentDay ) {
      upcomingForecast.push([forecastDay,iconURL, main, feels_like, forecastTime, dt_txt]);
    }
    if(Number(forecastDay) <= currentDay+1) {
      today.push([forecastDay,iconURL, main, feels_like, forecastTime, dt_txt]);
    }
...
useEffect(() => {
    async function getForecast() {
      const rawForecast = await fetch("https://api.openweathermap.org/data/2.5/forecast?lat="+latitude+"&lon="+longitude+"&appid=58ab3e4fa033b9933bab777dff79301b" , {mode:'cors'})
  ...
  if(forecast) {
    ...
    function handleClick(day){
      ...
      Array.from(expandElements).forEach(element => {
        switch(element.style.display) {
          case '':
          case 'none': element.setAttribute('style',show) ; return;
          case 'flex': element.style.display = 'none';return;
    ...
    const extendedForecast = upcomingForecast.map((item, index) => {
      ...
      if (index > 0 && upcomingForecast[index][0]  !== upcomingForecast[index-1][0]){
        dayAdded = false;
      }
