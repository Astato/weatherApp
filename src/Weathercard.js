...
const WeatherCards = (props) => {
    ...
    const getweather = async (location) => {
        const currentWeather = await fetch("https://api.openweathermap.org/data/2.5/weather?q="+location+"&appid=58ab3e4fa033b9933bab777dff79301b", { mode: 'cors' });
    ...
    function FilterSearch() {
        ...
        const getLocation = async (location) => {
            ...
            const searchForLocation = await fetch("http://api.openweathermap.org/geo/1.0/direct?q="+location+"&limit=5&appid=58ab3e4fa033b9933bab777dff79301b")
        ...
        if(isLocation && locationsData){
             const mapLocations = locationsData.map((item, index) => { 
                ...
                if(state) {
                    return <div key = {index} onClick={() => {setLocation(name,country,state); setIsSearch(false); citysearch.value = ''}}>
                        <p  className='locations-name' onClick={() => {setLocation(name, country); setIsSearch(false); citysearch.value = ''}}>{name}, {country}, {state}</p>
                    </div>
                }
                else{
                    return <div key={index} onClick={() => {setLocation(name, country); setIsSearch(false); citysearch.value = ''}}>
                    <p className='locations-name' onClick={() => {setLocation(name, country); setIsSearch(false); citysearch.value = ''}}>{name}, {country}</p>
                </div>
                }
            })
            return (
    ...
    function getDate(dt, timezone) {
        ...
        const localHours = new Date(localDate).getUTCHours() +':'+ new Date(localDate).getUTCMinutes();
        return localHours;
    }
    ...
    useEffect(() => {
        if((localHours > sunriseTime && localHours > sunsetTime) || (localHours <sunriseTime  && localHours < sunsetTime)) {
        ...
        return (
            <div id='weather-cards' >
                <div id='forecast-container'>
                    ...
                    <div onClick={() => handleUnitChange('C')} id='celsius'>C°</div>
                    <div onClick={() => handleUnitChange('F')}id='fahr'>F°</div>
                </div>
                <div id='secondary-info'>
                    <p>{country}, {name}}</p>
                    <img src={iconURL} alt={main}></img>
                    <p>{feelsTemp}{tempUnit}</p>
                </div>
                <div id='searchfield'>
                    <img src={searchIcon} id='search-icon' alt='search' onClick={()=> setIsSearch(true)}></img>
                    <input id='citysearch' type='text' placeholder='London, CA / Barcelona' onChange={(e) =>  {setLocationInput(e.target.value); setIsSearch(false)}} 
                    onKeyDown = {(e) => {if(e.code === 'Enter'){setIsSearch(true)}} }></input>
                    <div id='found-locations'>
                        <FilterSearch></FilterSearch>
                    </div>
                </div>
                <Forecast locationLon={longitude} locationLat ={latitude} setToday={setTodayForecast} units={tempUnit}></Forecast>
            </div>
                
                <div id='card' >
                    <div id='update-container'>
                        <p>{date}</p>
                        <img id='update-icon' onClick={() => {getweather(location); animate()}} src={updateIcon} alt='update'></img>
                    </div>
                    <div className='location'>
                        <div id='location-temperature' >
                            <p>{country}, {name}</p>
                            <p id='feels-like'>{feelsTemp} {tempUnit}</p>
                        ...
                        <div id='icon-description'>
                            ...
                            <div id='temperature'>
                                    <p className='temp-info'>{maxTemp} / {minTemp}{tempUnit}</p>
                            </div>
                    <div className='wind'>
